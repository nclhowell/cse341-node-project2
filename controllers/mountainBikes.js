...
const createSingle = async (req, res) => {
  ...
  const response = await mongodb.getDb().db("project2").collection("mountainBikes").insertOne(mountainBike);
  if (response.acknowledged) {
    res.status(201).json(response);
  } else {
    res.status(500).json(response.error || "Some error occurred while creating the mountainBike.");
  }
};
...
const deleteSingle = async (req, res) => {
  ...
  const bikeId = new ObjectId(req.params.id);
  const response = await mongodb.getDb().db("project2").collection("mountainBikes").deleteOne({ _id: bikeId }, true);
  console.log(response);
  if (response.deletedCount > 0) {
    res.status(204).send();
  } else {
    res.status(500).json(response.error || "Some error occurred while deleting the mountainBike.");
  }
};
